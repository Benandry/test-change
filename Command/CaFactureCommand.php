...
use App\Controller\CaFactureController;
use Symfony\Component\HttpClient\HttpClient;
use App\Entity\CaFacture;
use App\Entity\Stores;
use Symfony\Component\Console\Input\InputArgument;

class CaFactureCommand extends Command
{
    // the name of the command (the part after "bin/console")
    protected static $defaultName = 'app:all-ca-facture';

    private $container;
    private $CaFactureController;

    private $doctrine;

    public function __construct(
        ContainerInterface $container,
        CaFactureController $CaFactureController,
        ManagerRegistry $doctrine
    ) {
        parent::__construct();
        $this->container = $container;
        $this->CaFactureController = $CaFactureController;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($sinceId != null) {
            ...
            $lastInvoice =  $this->container->get('doctrine')->getRepository(CaFacture::class)->findOneBy(['store' => $storeId], array('id' => 'desc'), 1, 0);
            if ($lastInvoice) {
        ...
        $result = $this->CaFactureController->getAllInvoices($store, $lastInvoice);
        $allInvoices = $result['0'];
        $i = 0;
        if ($allInvoices) {
            foreach ($allInvoices as $saleInvoice) {
                $invoice = $this->container->get('doctrine')->getRepository(CaFacture::class)->findOneBy(['sinceId' => $saleInvoice['id']]);
                if (!$invoice) {
                    $i++;
                    $newSaleInvoice = new CaFacture();
                    ...
                    $newSaleInvoice->setGiftCards($gift);
                    ...
                    $newSaleInvoice->setCustomerBalance(0);
                    if (($storeId == 23) || ($storeId == 24) || ($storeId == 25)) {
